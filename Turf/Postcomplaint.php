...
if(isset($_POST["btn_submit"]))
{
	...
	if($id==""){
	$insQry="insert into tbl_complaint(complaint_title,complaint_content,complaint_date,owner_id)values('".$Title."','".$Content."',curdate(),'".$_SESSION["tid"]."')";
	if($con->query($insQry))
	{
	?>
	<script>
...
<table width="200" border="1">
  ...
  <tr>
     <?php
	$i=0;
	$sel="select * from tbl_complaint c inner join tbl_newowner h on c.owner_id=h.owner_id where c.owner_id='".$_SESSION['tid']."' and user_id=''";
	$row=$con->query($sel);
